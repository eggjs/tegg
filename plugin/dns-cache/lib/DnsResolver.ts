...
export class DnsResolver {
   ...
   * @param options.debug - Enable debug logging, default is false
   */
  constructor(options: DnsResolverOptions = {}, args: { logger: EggLogger }) {
    ...
    if (this.useResolver) {
      ...
      this.lookupPromises = {
        lookup: util.promisify(dns.lookup) as any,
      };
    }
  ...
  private _debugLog(msg: any, ...args: any[]) {
    if (this.debug) {
      this.logger.info.apply(this.logger, [msg, ...args]);
    }
  }
  ...
  private _initializeResolver(defaultServers?: string[]) {
    ...
    this.resolverPromises = this.resolver.promises || {
      resolve4: util.promisify(
        this.resolver.resolve4.bind(this.resolver)
      ) as any,
    };
  }
  ...
  private async _updateDNS(hostname: string): Promise<CacheEntry> {
    ...
    if (!this.useResolver) {
      try {
        this._debugLog(`[dns-cache] Starting dns.lookup for ${hostname}`);
    ...
    // Use dns.resolve
    try {
      this._debugLog(`[dns-cache] Starting dns.resolve4 for ${hostname}`);
